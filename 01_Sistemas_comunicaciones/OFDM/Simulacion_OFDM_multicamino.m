...
for k = 1:length(SNR)
    ...
    senyalCanal = awgn(senyalCanal,SNR(k),"measured");
    Salida_Canal = senyalCanal;

    %% Preambulo con ruido

    Entrenamiento_canal = filter(hcanal,1,preT);
    Entrenamiento_canal = awgn(Entrenamiento_canal,SNR(k),"measured");
    ...
    BerSnr_ruido(k) = Ber_ruido;

    %% Ecualizacion con estimacion ideal

    SimbOfdm_R_EQ_ideal = SimbOfdm_R .* EQ_ideal;

    Bits_R_ideal = qamdemod(SimbOfdm_R_EQ_ideal(:),MQAM,"bin","OutputType","bit","UnitAveragePower",true);

    [err_ideal,Ber_ideal] = biterr(Bits_T,Bits_R_ideal);

    BerSnr_ideal(k) = Ber_ideal;
...
figure
semilogy(SNR,BerSnr_ideal,'o-')
hold on
semilogy(SNR,BerSnr_ruido,'s-')
grid on
xlabel('SNR [dB]')
ylabel('BER')
legend('EQ ideal','EQ con ruido')
title('BER vs SNR - 16 QAM OFDM con canal multicamino')
